...
namespace sc.splinemesher.pro.editor
{
        ...
        private bool DrawDataPoint(Vector3 position, Vector3 tangent, Vector3 up, float3 inValue, out float3 outValue)
        {
            ...
            */
            ...
            if (GUIUtility.hotControl == id && math.abs(width.x - x.x) > 0f)
            {
                ...
                outValue.x = Mathf.Max(0.02f, outValue.x);
            ...
            if (GUIUtility.hotControl == id2 && math.abs(height.y - y.y) > 0f)
            {
                ...
                outValue.y = Mathf.Max(0.02f, outValue.y);
    ...
    public class ScaleToolUI : Overlay, ITransientOverlay
    {
        ...
        private Vector2Field gridSizeField;
        public override VisualElement CreatePanelContent()
        {
            ...
            gridSizeField = new Vector2Field("Grid Size")
            {
            ...
            {
                Vector3 gridSize = new Vector3(evt.newValue.x, evt.newValue.y, EditorSnapSettings.gridSize.z);
                EditorSnapSettings.gridSize = gridSize;
            });
            gridSizeField.style.minWidth = 275;
            
            root.Add(gridSizeField);
            
            EditorApplication.update += UpdateGridSizeField;
            
            return root;
        }
        
        private void UpdateGridSizeField()
        {
            if (gridSizeField != null)
            {
                gridSizeField.visible = ScaleTool.GridSnapping && EditorSnapSettings.gridSnapEnabled;
                gridSizeField.SetValueWithoutNotify(new Vector2(EditorSnapSettings.gridSize.x, EditorSnapSettings.gridSize.y));
            }
        }
    }
}