    ...
    public static class SplineCurveSettingsEditor
    {
        ...
        public class OutputMesh : UI.Section.SectionEditor
        {
            ...
            public override void OnInspectorGUI(ref bool changed)
            {
                ...
                if (SplineMesherSettings.GetCurrentPrecisionMode() == SplineMesherSettings.PrecisionMode.Half)
                {
                    if (segmentingMode.intValue == (int)CurveMeshSettings.OutputMesh.Segmenting.None ||
                        (segmentingMode.intValue == (int)CurveMeshSettings.OutputMesh.Segmenting.ByLength &&
                         maxSegmentLength.floatValue > 100))
                    {
                        EditorGUILayout.HelpBox("16-bit precision is used for vertex positions. Segments need to be shorter than ~100m to avoid visible distortions in the mesh", MessageType.Warning);
                    }
                }
