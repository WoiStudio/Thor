...
namespace sc.splinemesher.pro.editor
{
    public static partial class SplineMesherEditor
    {
        ...
        {
            public const string SPLINE_MESHER_HALF_PRECISION = "SPLINE_MESHER_HALF_PRECISION";
            
            public static void SetDefineSymbol(string define, bool add, BuildTargetGroup buildTargetGroup = BuildTargetGroup.Unknown)
            {
                try
                {
                    var targetGroups = System.Enum.GetValues(typeof(BuildTargetGroup))
                        .Cast<BuildTargetGroup>()
                        .Where(btg => btg != BuildTargetGroup.Unknown).ToList();
                    ...
                    foreach (BuildTargetGroup targetPlatform in targetGroups)
                    {
                        //Eg. WebPlayer or Tirzen
                        if(IsObsolete(targetPlatform)) continue;
                        
                        var namedBuildTarget = NamedBuildTarget.FromBuildTargetGroup(targetPlatform);

                        //If 'Unknown' is specified, add to all platforms. Otherwise, only add to the specified platform.
                        if (buildTargetGroup != BuildTargetGroup.Unknown && targetPlatform != buildTargetGroup)
                            continue;
...
#if SM_DEV
                        Debug.Log(
                            $"Setting define symbol \"{define}\" to {(add ? "true" : "false")} for {namedBuildTarget.TargetName}");
#endif
                    ...
                    EditorUtility.DisplayDialog("Spline Mesher Pro", $"Precision mode for vertices will be changed after a domain reload." +
                                                                     "\n\n" +
                                                                     "This configuration is project-wide, be sure to commit the \"ProjectSettings.asset\" file to source control.",
                        "Ok");
                }
                catch (Exception ex)
                {
                    Debug.LogError($"Error in SetDefineSymbol: {ex}");
                }
            }
        }
    }
