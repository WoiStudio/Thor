...
namespace sc.splinemesher.pro.runtime
{
    public partial class SplineCurveMesher
    {
        ...
        {
            //Trim excess data entries
            void RemoveExtraData<T>(ref List<SplineData<T>> data)
            {
                int dataCount = data.Count;

                if (dataCount > splineCount)
                {
                    for (int i = dataCount - 1; i >= splineCount; i--)
                    {
                        //Debug.Log($"Removing spline data entry {i} of type {typeof(T).Name} from {data}. Spline count:{splineCount}");
                        
                        data.RemoveAt(i);
                    }
                }
            }
            RemoveExtraData(ref scaleData);
            RemoveExtraData(ref rollData);
            RemoveExtraData(ref vertexColorRedData);
            RemoveExtraData(ref vertexColorGreenData);
            RemoveExtraData(ref vertexColorBlueData);
            RemoveExtraData(ref vertexColorAlphaData);
            RemoveExtraData(ref conformingData);
