...
namespace sc.splinemesher.pro.runtime
{
    ...
    public partial class SplineCurveMesher : SplineMesher, IDisposable
    {
        ...
        private void RebuildSpline(int splineIndex)
        {
            ...
            {
                Debug.LogError($"[Spline Curve Mesher] Spline #{splineIndex} in {splineContainer.name} length is NaN. Cannot generate mesh. This safeguard not only available in release builds and will lead to memory leaks, " +
        ...
        private void GenerateGeometry(NativeSpline nativeSpline, int splineIndex, SplineCurveMeshData splineMeshData, Structs.InputMeshData inputMeshData, bool createMesh, Structs.InputMeshData colliderData, bool createCollider)
        {
            ...
            int totalTiles = RecalculateTiles();
            
            float totalMeshLength = totalTiles * meshOccupiedLength;
            ...
            if (totalTiles <= 0)
            {
                Debug.LogWarning($"Spline Curve Mesher: No tiles generated. Check the settings and make sure the spline is long enough ({splineLength}) to generate at least one tile.", this);
                return;
            }
            
            totalMeshLength = totalTiles * meshOccupiedLength;
            ...
            {
                SplineMeshSegment segment = container.GetOrCreateSegment(i);
                ...
                #if UNITY_EDITOR
                segment.name = $"Mesh Segment #{i}";
                #endif
                
                //Calculate how many tiles this segment should contain
                int tilesPerSegment = Mathf.FloorToInt((float)totalTiles / segmentCount);
                
                //Distribute remainder tiles across segments
                if (i < (totalTiles % segmentCount))
                {
                    tilesPerSegment += 1;
                }
                ...
                void CalculateSegmentLength()
                {
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        int prevSegmentTiles = Mathf.FloorToInt((float)totalTiles / segmentCount);
                        if (j < (totalTiles % segmentCount))
                        {
                            prevSegmentTiles += 1;
                        }
                ...
                segment.curveInfo = new Structs.CurveSegmentInfo()
                {
                    index = i,
                    curveRange = curveRange,
                    tileCount = tilesPerSegment,
                    tileLength = meshLength,
                    tileOccupiedLength = meshOccupiedLength,
                };
                
                #if UNITY_EDITOR
                //Ensure changes made to the segment are serialized.
                UnityEditor.EditorUtility.SetDirty(segment);
                #endif

                if (segment.curveInfo.tileCount > 0)
                {
                    curveToMeshJobs[i].Setup(inputMeshData,  createMesh, colliderData, createCollider, nativeSpline, splinePoints, segment.curveInfo, splineLength, 
                ...
                {
                    #if SM_DEV
                    Debug.LogWarning($"Spline Curve Mesher: Segment {i} has no tiles. This is likely due to the spline being too short ({trimmedSplineLength}).", this);
                    #endif
                }
            }
            Profiler.EndSample();
            ...
            Profiler.BeginSample("Spline Curve Mesher: Create output meshes");
            {
                ...
                for (int i = 0; i < segmentCount; i++)
                {
                    ...
                    //Nothing created for it
                    if (segment.curveInfo.tileCount == 0)
                    {
                        #if SM_DEV
                        Debug.LogWarning($"Spline Curve Mesher: Segment {i} has no tiles. Skipping mesh generation", this);
                        #endif
                        continue;
                    }

                    segment.transform.localPosition = curveToMeshJobs[i].pivotOffset;
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
                        segmentMesh.name = $"{objName}_Spline{splineIndex}_Mesh_{i}";
#endif
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
                        collisionMesh.name = $"{objName}_Spline{splineIndex}_Collider_{i}";
#endif
