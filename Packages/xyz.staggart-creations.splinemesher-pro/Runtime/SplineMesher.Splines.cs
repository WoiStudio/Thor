...
namespace sc.splinemesher.pro.runtime
{
    public partial class SplineMesher
    {
        ...
        private void OnSplineChanged(Spline spline, int knotIndex, SplineModification modificationType)
        {
            ...
            var splineIndex = GetSplineIndex(spline);
            if (splineIndex < 0)
                return;
            ...
            {
                UpdateCachedSpline(spline, splineIndex);
                Rebuild(splineIndex);
            }
        }
        ...
        private void ExecuteAfterSplineChanges()
        {
            ...
            UpdateCachedSpline(lastEditedSpline, lastEditedSplineIndex);
            Rebuild();
        ...
        private void OnSplineAdded(SplineContainer container, int index)
        {
            ...
            AddCachedSpline(container.Splines[index]);
        ...
        /// <param name="forceCacheRebuild">Force the cache for all the splines to be rebuilt, set to false if you intend to call <see cref="Rebuild"/> afterwards.</param>
        ...
        /// <summary>
        /// Disposes all cached splines. This is already called from the <see cref="Dispose"/> function, but can be called manually if you want to dispose of the cached splines only.
        /// </summary>
        public void DisposeNativeSplines()
        {
        ...
        private int GetSplineIndex(Spline spline)
        {
            if(!splineContainer) return -1;
            
            return Array.IndexOf(splineContainer.Splines.ToArray(), spline);
        }
        
        /// <summary>
        /// Adds a spline to the cache (end of list)
        /// </summary>
        /// <param name="spline"></param>
        /// <param name="index"></param>
        public void AddCachedSpline(Spline spline, int index)
        {
            NativeSpline nativeSpline = CreateNativeSpline(spline);
            nativeSplines.Insert(index, nativeSpline);
        }
        ...
        /// </summary>
        /// <param name="spline"></param>
        public void AddCachedSpline(Spline spline)
        {
            NativeSpline nativeSpline = CreateNativeSpline(spline);
        ...
        /// <summary>
        /// Updates the cached spline. Use this if only manually rebuilding, in which case the component will not update spline
        /// caches automatically when they change or are added/removed. If no related cached spline is found, a new one is created.
        /// </summary>
        /// <param name="spline"></param>
        /// <param name="index"></param>
        public void UpdateCachedSpline(Spline spline)
        {
            var index = GetSplineIndex(spline);
            if (index < 0) return;
            
            UpdateCachedSpline(spline, index);
        }
        
        /// <summary>
        /// Updates the cached spline at the specified index. Use this if only manually rebuilding, in which case the component will not update spline
        /// caches automatically when they change or are added/removed. If no spline is found at the specified index, a new one is created.
        /// </summary>
        /// <param name="spline"></param>
        /// <param name="index"></param>
        public void UpdateCachedSpline(Spline spline, int index)
        {
            if (index >= nativeSplines.Count)
            {
                AddCachedSpline(spline);
        ...
        public void RemoveCachedSpline(Spline spline)
        {
            var index = GetSplineIndex(spline);
            if (index < 0) return;
        ...
        public void RemoveCachedSpline(int index)
        {
        ...
        /// <summary>
        /// Gets the cached spline at the specified index. If no spline is found at the specified index, a new one is created.
        /// </summary>
        /// <param name="index"></param>
        /// <returns></returns>
        public NativeSpline GetNativeSpline(int index)
        {
            ...
            if (index >= nativeSplines.Count)
            {
                ...
                {
                    AddCachedSpline(splineContainer.Splines[index]);
                }
            }
