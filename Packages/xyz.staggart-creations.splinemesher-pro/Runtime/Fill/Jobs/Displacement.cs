...
namespace sc.splinemesher.pro.runtime
{
    ...
    public struct Displacement : IJobParallelFor
    {
        ...
        public void Setup(FillMeshSettings.Displacement settings, FillMeshSettings.Conforming conforming, float4x4 worldToLocal, NativeArray<float3> points, NativeArray<Structs.PointState> pointStates, 
        ...
        public void Execute(int i)
        {
            ...
            if (settings.noise.y > 0)
            {
                float yOffset = noise.snoise(new float2(position.x + settings.noiseOffset.x, position.z + settings.noiseOffset.y) * settings.noiseFrequency.y * 0.1f) * settings.noise.y;

                //noiseDirection value is between -1 (negative only) and +1 (positive only)
                yOffset = math.lerp(yOffset, math.lerp(-math.abs(yOffset), math.abs(yOffset), settings.noiseDirection * 0.5f + 0.5f), math.abs(settings.noiseDirection));
                
                yOffset *= math.saturate(((distance / settings.noiseFalloff) / maxDistance));
