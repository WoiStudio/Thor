...
namespace sc.splinemesher.pro.runtime
{
    ...
    public struct CreateVertices : IJobParallelFor, IDisposable
    {
        ...
        private const float SQRT_3 = 1.7320f; //Sqrt(3)
        private const float EDGE_DISTANCE_THRESHOLD = 1.7320f; //Sqrt(3)
        
        public void Setup(int2 gridSize, float triangleSize, Bounds bounds, NativeSpline nativeSpline, bool clockwise, NativeArray<Structs.SplinePoint> splinePoints, SplineFillMesher mesher)
        {
            ...
            splineSearchSampleCount = (int)math.ceil(spline.GetLength() / (triangleSize * searchIntervalScalar));

            //Pre-calculate constants
            this.triangleSize = triangleSize;
            halfTriangleSize = triangleSize * 0.5f;
            triangleHeight = triangleSize * math.sin(math.PI / 3f);
            edgeDistanceThreshold = triangleSize * EDGE_DISTANCE_THRESHOLD;
        ...
        public static int2 GetGridSize(float triangleSize, float width, float height)
        {
            ...
            int zCount = Mathf.CeilToInt((height / (triangleSize * SQRT_3 * 0.5f))) + 1;
